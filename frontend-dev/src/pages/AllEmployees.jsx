...
function AllEmployees({ newFormId }) {
  ...
  const adminURL = bitwelzp.adminURL || bitwelzp.ajaxURL.replace('admin-ajax.php', '')
  ...
  const [cols, setCols] = useState(() => mergeSavedCols('all_employees', [
    {
      ...
      accessor: 'zoho_id',
      Cell: (e) => (e.row.original.page_status === 'active' ? (
        <a
          className="btcd-tabl-lnk"
          href={`https://wellqor.com/${e.row.original.fname.charAt(
            0,
          )}${filterLName(e.row.original.lname)}`}
          target="_blank"
          rel="noreferrer"
        >
          {e.row.original.zoho_id}
        </a>
      ) : (
        e.row.original.zoho_id
      )),
    },
    {
      width: 150,
      minWidth: 80,
      Header: __('Employee ID', 'bitwelzp'),
    ...
    },
    {
      width: 300,
      minWidth: 80,
      Header: __('Review Form Link', 'bitwelzp'),
      accessor: 'review_form_link',
      Cell: (e) => (
        <CopyText
          value={`https://wellqor.com/therapist-review-form/?zoho_id=${e.row.original.zoho_id}`}
    ...
    },
    {
      width: 250,
      minWidth: 80,
      Header: __('Employee Status', 'bitwelzp'),
      ...
      Cell: (e) => (e.row.original.page_status === 'active' ? (
        <a
          ...
          rel="noreferrer"
        >
          {e.row.original.employee_status}
        </a>
      ) : (
        e.row.original.employee_status
      )),
    ...
    },
    {
      width: 250,
      minWidth: 80,
      Header: __('Medical Qualification', 'bitwelzp'),
      accessor: 'medical_qualification',
      Cell: (e) => (e.row.original.page_status === 'active' ? (
        <a
          className="btcd-tabl-lnk"
          href={`https://wellqor.com/${e.row.original.fname.charAt(
          ...
          rel="noreferrer"
        >
          {e.row.original.medical_qualification}
        </a>
      ) : (
        e.row.original.medical_qualification
      )),
    },
    {
      width: 250,
      minWidth: 80,
      Header: __('Designation', 'bitwelzp'),
    ...
    },
    {
      width: 250,
      minWidth: 80,
      Header: __('Certifications', 'bitwelzp'),
    ...
    },
    {
      width: 250,
      minWidth: 80,
      Header: __('Cultural Competency', 'bitwelzp'),
      accessor: 'cultural_competency',
      Cell: (e) => (e.row.original.page_status === 'active' ? (
        <a
          className="btcd-tabl-lnk"
          href={`https://wellqor.com/${e.row.original.fname.charAt(
          ...
          rel="noreferrer"
        >
          {e.row.original.cultural_competency}
        </a>
      ) : (
        e.row.original.cultural_competency
      )),
    ...
    },
    {
      width: 250,
      minWidth: 80,
      Header: __('Licensed In', 'bitwelzp'),
      accessor: 'licensed_in',
      Cell: (e) => (e.row.original.page_status === 'active' ? (
        <a
          className="btcd-tabl-lnk"
          href={`https://wellqor.com/${e.row.original.fname.charAt(
      ...
      .then((result) => {
        ...
        if (result?.success) {
          if (result.data.length > tableData.length) {
            setSnackbar({
              show: true,
              msg: __('New Empolyee Added and Data Updated', 'bitwelzp'),
            })
          } else {
            setSnackbar({
              show: true,
          ...
          setSnackbar({
            show: true,
            msg: `${__('Data fetch failed Cause:', 'bitwelzp')}${
              result.data.data || result.data
            }. ${__('please try again', 'bitwelzp')}`,
          })
        } else {
          setSnackbar({
            show: true,
            msg: __('Data fetch failed. please try again', 'bitwelzp'),
          })
        }
      })
  ...
  useEffect(() => {
    if (!integConfig.auth_details.isAuthorized) {
      setSnackbar({
        show: true,
        msg: __('Please complete the authorization first', 'bitwelzp'),
      })
    }
  }, [])
  ...
  const DataFetchBtn = () => (
    <button
      ...
      disabled={!integConfig?.auth_details?.isAuthorized}
      onClick={fetchData}
      type="button"
