...
function AllEmployees({ newFormId }) {
  ...
  const [cols, setCols] = useState([
    ...
    // { width: 250, minWidth: 80, Header: __('Skills', 'bitwelzp'), accessor: 'skills', Cell: e =>e.row.original.page_status==='active'? <a className="btcd-tabl-lnk" href={`https://wellqor.com/${e.row.original.fname.charAt(0)}${e.row.original.lname}`} target="_blank">{e.row.original.skills}</a>:e.row.original.skills  },
  ...
  useEffect(() => {
    if (!integConfig.integ_config.auth_details.isAuthorized) {
  ...
  const DataFetchBtn = () => {
    return (
      <button style={{ marginLeft: '10px', marginRight: '10px' }} disabled={!integConfig.integ_config.auth_details.isAuthorized} onClick={fetchData} type="button" className="btn btcd-btn-lg green sh-sm flx">{__('Fetch data', 'bitwelzp')}
