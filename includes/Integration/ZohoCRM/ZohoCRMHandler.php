...
use BitCode\WELZP\Integration\ZohoCRM\MetaDataApiHelper;
use BitCode\WELZP\Integration\ZohoCRM\RecordApiHelper;
use BitCode\WELZP\Integration\IntegrationHandler;
...
final class ZohoCRMHandler
{
    ...
    {
        add_action('wp_ajax_bitwelzp_zcrm_get_users', array(__CLASS__, 'refreshUsersAjaxHelper'));
        add_action('wp_ajax_bitwelzp_zcrm_get_tags', array(__CLASS__, 'refreshTagListAjaxHelper'));
        add_action('wp_ajax_bitwelzp_zcrm_get_assignment_rules', array(__CLASS__, 'getAssignmentRulesAjaxHelper'));
        add_action('wp_ajax_nopriv_bitwelzp_zcrm_get_assignment_rules', array(__CLASS__, 'getAssignmentRulesAjaxHelper'));
        add_action('wp_ajax_bitwelzp_zcrm_get_related_lists', array(__CLASS__, 'getRelatedListsAjaxHelper'));
        add_action('wp_ajax_bitwelzp_zcrm_generate_token', array(__CLASS__, 'generateTokens'));
        add_action('wp_ajax_bitwelzp_zcrm_refresh_modules', array(__CLASS__, 'refreshModulesAjaxHelper'));
        add_action('wp_ajax_bitwelzp_zcrm_refresh_layouts', array(__CLASS__, 'refreshLayoutsAjaxHelper'));
    }
    ...
    public static function refreshModulesAjaxHelper()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bitcffp_nonce')) {
            ...
            if ((intval($queryParams->tokenDetails->generates_on) + (55 * 60)) < time()) {
                $refreshedToken = ZohoCRMHandler::_refreshAccessToken($queryParams);
                if ($refreshedToken) {
        ...
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce(sanitize_text_field($_REQUEST['_ajax_nonce']), 'bitcffp_nonce')) {
            ...
            if (!is_wp_error($layoutsMetaResponse) && (empty($layoutsMetaResponse->status) || (!empty($layoutsMetaResponse->status) && $layoutsMetaResponse->status !== 'error'))) {
                ...
                foreach ($retriveLayoutsData as $layoutKey => $layoutValue) {
                    global $isbitwelzpLicActive;
                    if (!$isbitwelzpLicActive && $layoutValue->name !== 'Standard') {
                        continue;
                    }
                    $fields = [];
                    $fileUploadFields = [];
                    ...
                    foreach ($layoutValue->sections as $sectionKey => $sectionValue) {
                        foreach ($sectionValue->fields as $fieldKey => $fieldDetails) {
                            if (!$isbitwelzpLicActive && !in_array($fieldDetails->api_name, $fieldToShow)) {
                                continue;
                            }
                            if (
    ...
    {
        add_filter('bitwelzp_addRelatedList', array(__CLASS__, 'addRelatedList'), 10, 6);
    }
    ...
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce($_REQUEST['_ajax_nonce'], 'bitcffp_nonce')) {
            $inputJSON = file_get_contents('php://input');
            $queryParams = json_decode($inputJSON);
            if (
                empty($queryParams->module)
            ...
            if (
                ...
                wp_send_json_error(
                    !empty($assignmentRulesResponse->status)
                        ...
                        $assignmentRulesResponse->message : (empty($assignmentRulesResponse) ? __('Assignment rules is empty', 'bitwelzp') : 'Unknown'),
            ...
            if (
                ...
                wp_send_json_error(
                    !empty($relatedListResponse->status)
                        ...
                        $relatedListResponse->message : (empty($relatedListResponse) ? __('RelatedList is empty', 'bitwelzp') : 'Unknown'),
    ...
    public static function refreshTagListAjaxHelper()
    {
        if (isset($_REQUEST['_ajax_nonce']) && wp_verify_nonce($_REQUEST['_ajax_nonce'], 'bitcffp_nonce')) {
            ...
            if (!is_wp_error($tagListApiResponse)) {
                ...
                wp_send_json_error(
                    is_wp_error($tagListApiResponse) ? $tagListApiResponse->get_error_message() : (empty($tagListApiResponse) ? __('Tag is empty', 'bitwelzp') : 'Unknown'),
    ...
    public static function addRelatedList($zcrmApiResponse, $formID, $integID, $fieldValues, $integrationDetails, RecordApiHelper $recordApiHelper)
    {
        foreach ($integrationDetails->relatedlists as $relatedlist) {
            ...
            if (empty($relatedListModule) || empty($relatedListLayout)) {
                return new WP_Error('REQ_FIELD_EMPTY', __('module, layout are required for zoho crm relatedlist', 'bitwelzp'));
            }
    ...
    public function execute(IntegrationHandler $integrationHandler, $integrationData, $fieldValues)
    {
        ...
        if (
            ...
            $error = new WP_Error('REQ_FIELD_EMPTY', __('module, layout, fields are required for zoho crm api', 'bitwelzp'));
        ...
        if ((intval($tokenDetails->generates_on) + (55 * 60)) < time()) {
            ...
            if ($newTokenDetails) {
                ...
                Log::save($this->_formID, $this->_integrationID, 'token', 'error', 'Failed to refresh access token');
                return;
            }
        }
        ...
        if (
            ...
            $zcrmApiResponse = apply_filters('bitwelzp_addRelatedList', $zcrmApiResponse, $this->_formID, $this->_integrationID, $fieldValues, $integrationDetails, $recordApiHelper);
        }
        return $zcrmApiResponse;
    }
}
